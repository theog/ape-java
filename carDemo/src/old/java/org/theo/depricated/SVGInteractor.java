...
square.setAttributeNS (null, "opacity", "0.7");
 ...
 * to those elements in the document which we want
 * to make interactive
 *
 */
public void registerListeners () {
   ...
   // EventTarget
   EventTarget t1 =
      (EventTarget) document.getElementById ("theCircle");
...
t1.addEventListener ("SVGLoad",
  new EventListener () {
     public void handleEvent (Event evt) {
        window.setInterval (new CircleMovement (), 20);
        window.setInterval (new SquareMovement (), 15);
      }
   ...
   EventTarget t2 =
      (EventTarget) document.getElementById ("theSquare");
...
// An inner class encapsulating the laws of the circle's
   // movement
   public class CircleMovement implements Runnable {
      ...
      public void run () {
         ...
         if ((yPos <= 70) || (yPos >= 330)) deltaY = -deltaY;
         yPos += deltaY;
  ...
  public class SquareMovement implements Runnable {
    ...
    public void run () {
       ...
       if ((xPos <= 0) || (xPos >= 440)) deltaX = -deltaX;
       xPos += deltaX;
